...
//   const state = {
//     ...(sanitizedGroupBy !== undefined ? { grouping: sanitizedGroupBy } : {}),
...
type BaseTableProps<T extends object> = {
  ...
  /** Array of row data */
  data: T[];
  /** Stable row identity used by sorting, grouping, and selection */
...
const BaseTable = <T extends object>({
  ...
  const hasSelectableRows =
    typeof enableRowSelection === "function"
      ? data.some(row => enableRowSelection(row))
      : enableRowSelection;
  ...
  const table = useMaterialReactTable<T>({
    ...
    enableStickyHeader: true,
    enableRowSelection:
      hasSelectableRows && typeof enableRowSelection === "function"
        ? ({ original }) => enableRowSelection(original)
        : hasSelectableRows,
    muiSelectCheckboxProps: ({ row }) => ({
      sx: row.getCanSelect() ? undefined : { display: "none" },
    }),
    selectAllMode: "all",
    onRowSelectionChange,
