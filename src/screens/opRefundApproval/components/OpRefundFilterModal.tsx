...
export type OpRefundApprovalFilterValues = {
  branchId: number;
  fromDate: string;
  toDate: string;
};
...
const OpRefundFilterModal = ({
  isOpen,
  onClose,
  onApply,
  initialValues,
  branchList = [],
  modalTitle = "Filter OP Refund Approval",
}: {
  isOpen: boolean;
  onClose: () => void;
  onApply: (values: OpRefundApprovalFilterValues) => void;
  initialValues: OpRefundApprovalFilterValues;
  branchList?: BranchOption[];
  modalTitle?: string;
}) => {
  const [filterValues, setFilterValues] = useState<OpRefundApprovalFilterValues>(initialValues);
...
export default OpRefundFilterModal;
