...
const SERVICE_DEFAULTS: Record<string, { code: string; rate: number; tax: number; category: string; type: string }> = {
  "Deluxe Room (Per Day)": { code: "IPD-ROOM-001", rate: 5000, tax: 0, category: "Room", type: "Charge" },
  "Consultation": { code: "DOC-001", rate: 1500, tax: 18, category: "Doctor", type: "Service" },
  "2D Echocardiography": { code: "PROC-002", rate: 2800, tax: 18, category: "Procedure", type: "Procedure" },
  "Complete Blood Count (CBC)": { code: "INV-001", rate: 600, tax: 18, category: "Investigation", type: "Investigation" },
  "Paracetamol 650mg": { code: "MED-045", rate: 15, tax: 18, category: "Medicine", type: "Medicine" },
  "IV Set": { code: "CON-021", rate: 250, tax: 18, category: "Consumable", type: "Consumable" },
  "Nursing Charges": { code: "MIS-010", rate: 800, tax: 0, category: "Miscellaneous", type: "Miscellaneous" },
};
...
const IpdBillingSummary = ({ patient }: { patient: IpdPatientItem }) => {
  ...
  const [activeCategoryTab, setActiveCategoryTab] = useState("Service");
      ...
      <div className="grid grid-cols-12 gap-5 items-start">
        ...
        <div className="col-span-12 lg:col-span-9 flex flex-col gap-5">
          ...
          <div className="bg-white border border-slate-200/70 rounded-2xl shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-4 flex flex-col gap-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-slate-800 font-bold text-sm uppercase tracking-wider flex items-center gap-2">
                <i className="fa-solid fa-file-invoice-dollar text-[#0B5394]"></i>
                Add Billing Item
              </h3>
            </div>

            {/* Category Tabs Selection */}
            <div className="flex flex-wrap items-center gap-2 border-b border-slate-100/70 pb-3">
              {CATEGORY_TABS.map(tab => {
                ...
                return (
                  <button
                    key={tab.id}
                    type="button"
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-2 transition duration-200 ${
                      isActive
                        ? "bg-[#0B5394] text-white shadow-sm shadow-[#0B5394]/15"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 mt-1">
              ...
              <InputField label="Service / Item" required>
                <div className="flex items-center gap-1.5">
                  ...
                  >
                    {Object.keys(SERVICE_DEFAULTS).map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                  <button
                    type="button"
                    className="w-9 h-9 bg-slate-100 border border-slate-200 rounded-lg hover:bg-slate-200 transition text-[#0B5394] font-bold text-lg flex items-center justify-center shrink-0"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
              ...
              <InputField label="Rate (₹)" required>
                <input
                  type="number"
                  className="input-field text-right font-medium text-slate-800"
                  value={rate}
                  min={0}
              ...
              <InputField label="Discount (%)">
                <input
                  type="number"
                  className="input-field text-right font-medium text-slate-600"
              ...
              <InputField label="Amount (₹)">
                <input
                  type="text"
                  className="input-field text-right font-bold text-[#0B5394] bg-slate-50/50"
                  value={formatCurrency(computedFormAmount)}
                  readOnly
                  disabled
                />
              </InputField>
            ...
            <div className="flex items-center justify-end mt-1 border-t border-slate-100 pt-3">
              <button
                type="button"
                className="px-5 py-2.5 bg-[#0B5394] text-white font-bold text-xs uppercase tracking-wider rounded-xl hover:bg-[#094376] shadow-sm shadow-[#0B5394]/10 transition duration-200"
          ...
          <div className="bg-white border border-slate-200/70 rounded-2xl shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-4 flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
              ...
              <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                ...
                <button type="button" className="px-3 py-1.5 bg-[#0B5394] text-white text-xs font-bold rounded-lg hover:bg-[#094376]">
                  Search
                </button>
                <button type="button" className="px-3 py-1.5 bg-white border border-slate-200 text-slate-600 text-xs font-bold rounded-lg hover:bg-slate-50">
                  Hold Bill
                </button>
                <button type="button" className="px-3 py-1.5 bg-white border border-slate-200 text-slate-600 text-xs font-bold rounded-lg hover:bg-slate-50">
                  Estimate
                </button>
                <button type="button" className="px-3 py-1.5 bg-white border border-slate-200 text-slate-500 text-xs font-semibold rounded-lg hover:bg-slate-50">
                  Clear Filters
                </button>
                <div className="h-6 w-[1px] bg-slate-200 mx-1 hidden xl:block" />
                <button type="button" className="px-3 py-1.5 bg-white border border-slate-200 text-[#0B5394] text-xs font-bold rounded-lg hover:bg-slate-50 ml-auto sm:ml-0">
                  Import Charges
                </button>
                <button type="button" className="px-3 py-1.5 bg-white border border-slate-200 text-[#0B5394] text-xs font-bold rounded-lg hover:bg-slate-50">
                  Add Note
                </button>
              </div>
            </div>
            ...
            {/* Table Area */}
            <div className="w-full overflow-x-auto border border-slate-200/60 rounded-xl hide-scrollbar">
              <table className="w-full border-collapse text-left text-xs">
                  ...
                  {billingItems.map(item => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="py-2.5 px-3 whitespace-nowrap text-slate-500">{item.dateTime}</td>
                        <td className="py-2.5 px-3 whitespace-nowrap font-medium">{item.category}</td>
                        <td className="py-2.5 px-3 whitespace-nowrap">{item.department}</td>
                        <td className="py-2.5 px-3 font-semibold text-slate-800">{item.serviceItem}</td>
                        <td className="py-2.5 px-3 whitespace-nowrap text-slate-500 font-mono">{item.code}</td>
                        <td className="py-2.5 px-3 whitespace-nowrap font-medium">{item.doctor}</td>
                        ...
                        <td className="py-2.5 px-3 text-right text-slate-500">{formatCurrency(disc)}</td>
                        <td className="py-2.5 px-3 text-right text-slate-500">{formatCurrency(tax)}</td>
                        <td className="py-2.5 px-3 text-right font-bold text-slate-900">{formatCurrency(item.amount)}</td>
                        <td className="py-2.5 px-3 whitespace-nowrap"><span className="text-slate-500 font-medium">{item.type}</span></td>
                        ...
                        <td className="py-2.5 px-3 text-center whitespace-nowrap">
                          <div className="flex items-center justify-center gap-1.5">
                            <button type="button" className="w-6 h-6 hover:bg-slate-100 rounded flex items-center justify-center text-slate-400 hover:text-blue-500" title="View details">
                              <i className="fa-regular fa-eye" />
                            </button>
                            <button type="button" className="w-6 h-6 hover:bg-slate-100 rounded flex items-center justify-center text-slate-400 hover:text-amber-500" title="Edit row">
                ...
                <tfoot>
                  <tr className="bg-slate-50 border-t border-slate-200/80 font-bold text-slate-800">
                    <td className="py-3 px-3" colSpan={6}>Total Items: {billingItems.length}</td>
                    <td className="py-3 px-3 text-right text-slate-400">--</td>
                    <td className="py-3 px-3 text-right text-slate-400">--</td>
                    <td className="py-3 px-3 text-right text-slate-500">{formatCurrency(tableTotals.discount)}</td>
                    <td className="py-3 px-3 text-right text-slate-500">{formatCurrency(tableTotals.tax)}</td>
                    <td className="py-3 px-3 text-right text-slate-900 font-extrabold">{formatCurrency(tableTotals.amount)}</td>
              ...
              <div className="overflow-x-auto w-full hide-scrollbar">
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
                    ...
                    <tr>
                      <td className="py-2 text-slate-800 font-semibold text-red-600">Consumables</td>
                ...
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
                    <tr>
                      ...
                      <td className="py-2 font-semibold text-blue-600 whitespace-nowrap cursor-pointer">ADV-10245</td>
                    ...
                    <tr>
                      ...
                      <td className="py-2 font-semibold text-blue-600 whitespace-nowrap cursor-pointer">ADV-10311</td>
                    ...
                    <tr>
                      ...
                      <td className="py-2 font-semibold text-blue-600 whitespace-nowrap cursor-pointer">ADV-10482</td>
                    ...
                    <tr>
                      ...
                      <td className="py-2 font-semibold text-blue-600 whitespace-nowrap cursor-pointer">ADV-10523</td>
              ...
              <div className="flex flex-col gap-2.5 max-h-[220px] overflow-y-auto hide-scrollbar pr-0.5">
                <div className="flex items-center justify-between text-xs font-semibold">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-slate-400 uppercase font-bold text-[9px] bg-slate-100 px-1.5 py-0.5 rounded">UPI</span>
                ...
                <div className="flex items-center justify-between text-xs font-semibold">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-slate-400 uppercase font-bold text-[9px] bg-slate-100 px-1.5 py-0.5 rounded">Card</span>
                ...
                <div className="flex items-center justify-between text-xs font-semibold">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-slate-400 uppercase font-bold text-[9px] bg-slate-100 px-1.5 py-0.5 rounded">Cash</span>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mt-1 bg-slate-50 border border-slate-200/80 p-3 rounded-2xl">
            <div className="flex items-center gap-2">
              <button type="button" className="px-4 py-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 font-bold text-xs rounded-xl uppercase tracking-wider transition">
                Cancel Bill
              </button>
              <button type="button" className="px-4 py-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 font-bold text-xs rounded-xl uppercase tracking-wider transition">
                Save Draft
            ...
            <div className="flex items-center gap-2">
              <button type="button" className="px-4 py-2 border border-[#0B5394] bg-white hover:bg-blue-50/50 text-[#0B5394] font-bold text-xs rounded-xl uppercase tracking-wider transition">
                Request Approval
              </button>
              <button type="button" className="px-4 py-2 border border-[#0B5394] bg-white hover:bg-blue-50/50 text-[#0B5394] font-bold text-xs rounded-xl uppercase tracking-wider transition">
                Collect Payment
              ...
              <button type="button" className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs rounded-xl uppercase tracking-wider shadow-sm transition">
          ...
          <div className="bg-white border border-slate-200/70 rounded-2xl shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-4 flex flex-col gap-3.5">
            ...
            <div className="flex flex-col gap-2 text-xs font-semibold text-slate-600">
              <div className="flex items-center justify-between">
                ...
                <span className="text-slate-900 font-bold">{formatCurrency(cumulativeSummary.grossBill)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span>Discount</span>
                <span className="text-slate-900 font-bold">{formatCurrency(cumulativeSummary.discount)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span>Tax</span>
                <span className="text-slate-900 font-bold">{formatCurrency(cumulativeSummary.tax)}</span>
              </div>
              ...
              <div className="border-t border-slate-100/70 pt-2.5 flex items-center justify-between text-emerald-600">
                ...
                <span className="font-bold">-{formatCurrency(cumulativeSummary.packageDiscount)}</span>
              </div>
              
              <div className="flex items-center justify-between border-t border-slate-100 pt-2 font-bold text-slate-800 text-sm">
                <span>Payable Amount</span>
                <span className="text-slate-900 font-extrabold">{formatCurrency(payableAmount)}</span>
              </div>
              ...
              <div className="flex items-center justify-between border-t border-slate-200/80 pt-2.5 font-extrabold text-red-600 text-sm">
                <span>Balance (₹)</span>
                <span className="font-extrabold text-red-600 text-base">{formatCurrency(balanceOutstanding)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200/70 rounded-2xl shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-4 flex flex-col gap-3.5">
            ...
            <div className="flex flex-col gap-2.5 text-xs font-semibold text-slate-600">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="text-[#0B5394] font-bold text-right max-w-[65%]">Cardiac Care Package</span>
              </div>
              <div className="flex items-center justify-between">
          ...
          <div className="bg-white border border-slate-200/70 rounded-2xl shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-4 flex flex-col gap-3">
            ...
            <div className="grid grid-cols-3 gap-2 mt-0.5">
              ...
              >
                ...
                <span className="text-[10px] font-bold text-slate-700 whitespace-nowrap">Bank Transfer</span>
              </button>
              <button 
                type="button" 
                className="flex flex-col items-center justify-center gap-1.5 p-2 bg-slate-50 hover:bg-slate-100/80 border border-slate-200/50 rounded-xl hover:border-[#0B5394]/30 hover:shadow-[0_2px_8px_rgba(11,83,148,0.05)] transition-all duration-200 group text-center"
