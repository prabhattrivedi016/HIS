...
const CorporateTransferApprovalViewDetailsPopup = ({
  isOpen,
  item,
  onClose,
}: {
  ...
  const getDetails = async () => {
    ...
    try {
      const resp = await fetchApi(
        ...
        { component: "CorporateTransferApprovalViewDetailsPopup" }
      );
      if (resp?.data?.[0]) {
...
export default React.memo(CorporateTransferApprovalViewDetailsPopup);
