...
const VitalHistory = ({ isOpen, vitalsList, vitalRecords }: VitalHistoryProps) => {
  ...
  useEffect(() => {
    const h = (e: MouseEvent) => {
      ...
      if (vitalTypeDropRef.current && !vitalTypeDropRef.current.contains(e.target as Node)) setShowVitalTypeDrop(false);
    };
  ...
  const vitalTypeLabel =
    selectedVitalTypes.length === 0 ? "Select Vital" :
    selectedVitalTypes.length === vitalsList.length ? "Select All" :
    selectedVitalTypes.length === 1 ? selectedVitalTypes[0] :
                  ...
                  {showVitalTypeDrop && (
                    <div className="absolute top-full left-0 mt-1 w-52 bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto" style={{ zIndex: 999 }}>
                      <label className="flex items-center gap-2 px-3 py-2 hover:bg-gray-50 cursor-pointer border-b text-xs font-semibold text-gray-700">
                        <input
                          type="checkbox"
                          checked={selectedVitalTypes.length === vitalsList.length && vitalsList.length > 0}
              ...
              <div className="table-size">
                <table className="base-table">
                  ...
                  <tbody>
                    {pagedRows.length === 0 ? (
                      <tr>
                        <td colSpan={activeVitalColumns.length + 1} className="table-empty">No vital records found</td>
                      </tr>
                    ) : (
                      pagedRows.map((row) => (
