...
import { AllergySection } from "../types";
...
// interface AllergyPatientOptions {
//   id: number;
//   name: string;
//   snomedCode: string;
//   VitalName: string;
//   vitalId: number;
//   vitalSection :<Record<<string>;
...
const AllergyPanel = ({
      ...
      fetchApi(
        ...
        setAllergenOptions(
          data.map((s: any, i: number) => ({
            id: Number(
              s.SaltId ?? s.saltId ?? s.SaltMasterId ?? s.saltMasterId ?? s.Id ?? s.id ?? i + 1
            ),
  ...
  const handleConfirmDelete = () => {
    ...
    setAllergyRecords(prev => prev.filter(r => r.id !== pendingDeleteId));
  ...
  const handleNotKnownAllergyChange = (checked: boolean) => {
  ...
  const handleSave = () => {
    ...
    setAllergyRecords(prev =>
      editId > 0 ? prev.map(r => (r.id === editId ? record : r)) : [...prev, record]
    );
    showSuccess(editId > 0 ? "Updated" : "Added");
    resetForm();
  };
  ...
  const handleClose = () => {
    onBind?.({
      summary: allergySummary || null,
      notKnownAllergy,
      records: allergyRecords.map(r => ({
        id: r.id,
        allergyId: r.allergyId,
        allergyName: r.allergy,
        allergyTypeId: r.allergyTypeId,
        allergyType: r.allergyType,
        reaction: r.reaction,
        remarks: r.remarks,
        interactionSeverity: r.severity,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-3">
            ...
            <div className="border-t pt-3 shrink-0">
              <InputField label="De-Activation Remarks" required={pendingDeleteId > 0}>
                <textarea
                  ...
                  placeholder={
                    pendingDeleteId > 0
                      ? "Required — enter remarks to confirm delete"
                      : "Enter de-activation remarks if applicable"
                  }
              ...
              {pendingDeleteId > 0 && (
                <div className="flex gap-2 mt-1">
                  <button
                    type="button"
                    className="save-btn flex-1 !bg-red-500 hover:!bg-red-600"
                    onClick={handleConfirmDelete}
                  >
                    Confirm Delete
                  </button>
                  <button type="button" className="cancel-button" onClick={handleCancelDelete}>
                    Cancel
                  </button>
                </div>
              )}
      ...
      {showAllergyMaster && <AllergyMasterPopup onClose={() => setShowAllergyMaster(false)} />}
    </div>
  );
};
