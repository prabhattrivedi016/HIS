...
const OpRefundPaymentActionPopup = ({
  refundId,
  rawItemMap,
  anchorRect,
  onClose,
  ...
  onClose: () => void;
  onView: (item: OPRefundPaymentItem) => void;
  onCollectPayment: (item: OPRefundPaymentItem) => void;
  onCancel: (item: OPRefundPaymentItem) => void;
}) => {
  const popupRef = useRef<HTMLDivElement>(null);
  const item = getOpRefundPaymentItemById(rawItemMap, refundId);
  const position = useAnchoredFixedPopupPosition(anchorRect, popupRef, refundId);
...
export default OpRefundPaymentActionPopup;
