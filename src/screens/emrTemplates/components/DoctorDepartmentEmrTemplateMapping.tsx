...
import { DepartmentItem, DoctorItem } from "@/screens/consultationHeaderMaster/types";
import { SelectItem } from "@/types";
import { showError, showSuccess, showWarning } from "@/utils/alert";
import { useMutation, useQuery } from "@tanstack/react-query";
import { useEffect, useMemo, useState } from "react";
import Select from "react-select";
import { TemplateMappingTableItem } from "../types";

interface TemplateMappingPayload {
  typeId: number;
  typeName: string;
  relatedToId: number;
...
const DoctorDepartmentEmrTemplateMapping = () => {
  ...
  const [editableTableItem, setEditableTableItem] = useState<TemplateMappingTableItem[]>([]);

  const [draggedItem, setDraggedItem] = useState<TemplateMappingTableItem | null>(null);
  ...
  const { data: allTemplates = [] } = useQuery<TemplateMappingTableItem[]>({
    queryKey: ["getAllTemplatesForMapping"],
    queryFn: getAllTemplates,
  });

  useEffect(() => {
    setEditableTableItem(allTemplates);
  ...
  const getDoctorsList = async () => {
    const resp = await fetchApi(
      ...
      { component: "DoctorDepartmentEmrTemplateMapping" }
    );
    return resp?.data;
  };
  ...
  const searchHandler = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data: TemplateMappingTableItem[] = raw
      .map(m => ({
  ...
  const handleDragStart = (item: TemplateMappingTableItem) => {
    setDraggedItem(item);
  };
  ...
  const createPayload = (): TemplateMappingPayload => {
  ...
  const updateMappingMutation = useMutation({
    mutationFn: async (payload: TemplateMappingPayload) => {
      const resp = await fetchApi(
        "POST",
        ENDPOINTS.SAVE_TEMPLATE_DEPARTMENT_MAPPING,
        payload,
        {},
        { component: "DoctorDepartmentEmrTemplateMapping" }
      );
    ...
    <>
      ...
      <div className="card mt-1">
        <div className="card-header">
          <h2 className="card-title">Doctor Department EMR Template Mapping List</h2>
        </div>
              ...
              <table className="base-table">
                <thead className="table-head">
                  <tr>
                    ...
                    <th className="table-th align-top">Template Name</th>
                ...
                <tbody>
                  ...
                  {editableTableItem.map((item: TemplateMappingTableItem, idx: number) => (
                    <tr
                      key={idx}
                      className="table-row"
                      draggable
...
export default DoctorDepartmentEmrTemplateMapping;
