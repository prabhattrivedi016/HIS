...
import {
  HistoPendingReasonMasterFormItem,
  histoPendingReasonMasterSchema,
...
const HistoPendingReasonMaster = () => {
  ...
  const getPendingReasonList = async () => {
    const resp = await fetchApi(
      "GET",
      ENDPOINTS.GET_HISTO_PENDING_REASON_MASTER,
      {},
      {},
      { component: "HistoPendingReasonMaster" }
    );
    return resp?.data ?? [];
  };
  ...
  const filteredPendingReason = useMemo(() => {
    ...
    return pendingReasonList.filter(item =>
      item.pendingReason.toLowerCase().includes(searchValue.trim().toLowerCase())
    );
  ...
  const createUpdateHistoPendingReasons = async (data: HistoPendingReasonMasterFormItem) => {
    const resp = await fetchApi(
      "POST",
      ENDPOINTS.CREATE_UPDATE_HISTO_PENDING_REASON_MASTER,
      data,
      {},
      { component: "HistoPendingReasonMaster" }
    );
          ...
          <div className="form-grid-4">
            <InputField label="Pending Reason" required>
              ...
              {errors.pendingReason && (
                <p className="input-field-error">{errors.pendingReason.message}</p>
              )}
            </InputField>
              ...
              <table className="base-table">
                ...
                <tbody>
                  {filteredPendingReason.length === 0 && (
                    <tr>
                      <td
                        colSpan={HistoPendingReasonMasterTableHeader.length}
