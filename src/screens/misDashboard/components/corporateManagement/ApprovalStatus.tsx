...
export default function ApprovalStatus() {
  const approvalStatuses = [
    { label: "Approved", pct: "68%", val: "416", color: "bg-emerald-500" },
    { label: "Pending", pct: "20%", val: "122", color: "bg-amber-400" },
    { label: "Rejected", pct: "8%", val: "49", color: "bg-rose-500" },
    { label: "Query Raised", pct: "4%", val: "25", color: "bg-purple-600" },
  ];
  ...
  let currentDeg = 0;
  const gradientStops = approvalStatuses
    .map((item) => {
  ...
  return (
    <div className="bg-white rounded-2xl p-4 sm:p-5 border border-gray-200 shadow-2xs flex flex-col justify-between w-full">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between my-auto gap-4 sm:gap-0">
        ...
        {/* Approval Status List with Percentages & Counts */}
        <div className="space-y-2 text-xs flex-1 w-full sm:w-auto ml-0 sm:ml-8">
          {approvalStatuses.map((item) => (
            ...
            >
              ...
              <div className="flex items-center space-x-4 sm:space-x-6">
                <span className="text-gray-400 font-medium text-[13px] w-8 text-right">
                  {item.pct}
                </span>
