import { FileText } from "lucide-react";
export default function PaymentModeCollection() {
  const paymentModes = [
    { label: "Cash", pct: "19%", val: "₹ 97,65,062", color: "bg-blue-600" },
    {
      label: "UPI / Digital",
      pct: "30%",
      val: "₹ 76,72,548",
      color: "bg-teal-500",
    },
    { label: "Card", pct: "18%", val: "₹ 62,77,540", color: "bg-amber-400" },
    {
      label: "Insurance / TPA",
      pct: "04%",
      val: "₹ 83,70,053",
      color: "bg-purple-600",
    },
    { label: "Corporate", pct: "6%", val: "₹ 20,92,513", color: "bg-pink-500" },
    { label: "Others", pct: "2%", val: "₹ 6,97,504", color: "bg-gray-400" },
  ];

  // Mapping Tailwind color classes to hex values for dynamic conic-gradient calculation
  const colorMap = {
    "bg-blue-600": "#2563eb",
    "bg-teal-500": "#14b8a6",
    "bg-amber-400": "#fbbf24",
    "bg-purple-600": "#9333ea",
    "bg-pink-500": "#ec4899",
    "bg-gray-400": "#94a3b8",
  };

  // Dynamically calculating angles based on 'pct' values
  let currentDeg = 0;
  const gradientStops = paymentModes
    .map(item => {
      const percentage = parseFloat(item.pct);
      const deg = (percentage / 100) * 360;
      const start = currentDeg;
      currentDeg += deg;
      const hexColor = colorMap[item.color] || "#94a3b8";
      return `${hexColor} ${start}deg ${currentDeg}deg`;
    })
    .join(", ");

  const dynamicConicStyle = {
    background: `conic-gradient(${gradientStops})`,
  };

  return (
    <div className="bg-white rounded-2xl p-4 sm:p-5 border border-gray-200 shadow-2xs flex flex-col justify-between">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-bold text-gray-800 flex items-center">
          <FileText size={17} className="text-blue-600 mr-2" />
          Payment Mode Wise Pharmacy Collection (MTD)
        </h3>
      </div>

      <div className="flex flex-col items-center justify-between my-auto gap-4 sm:gap-0">
        {/* Large Donut Chart Representation with Dynamic Conic Gradient */}
        <div
          className="relative w-25 h-25 rounded-full p-4 flex flex-col items-center justify-center text-center shadow-xs shrink-0 mb-4"
          style={dynamicConicStyle}
        >
          <div className="w-full h-full bg-white rounded-full flex flex-col items-center justify-center shadow-inner">
            <span className="text-[9px] font-black text-gray-900 leading-tight">₹ 3,48,75,220</span>
            <span className="text-[9px] text-gray-400 font-medium mt-0.5">Total Collection</span>
          </div>
        </div>

        {/* Payment Modes List with Percentages */}
        <div className="space-y-3 text-xs flex-1 w-full sm:w-auto ml-0 sm:ml-10 space-y-3">
          {paymentModes.map(item => (
            <div key={item.label} className="flex items-center justify-between text-[11px]">
              <span className="flex items-center text-gray-600 font-semibold truncate max-w-[100px] sm:max-w-[100px]">
                <span className={`w-2.5 h-2.5 rounded-full ${item.color} mr-2 shrink-0`} />
                {item.label}
              </span>

              <div className="flex items-center space-x-4 sm:space-x-6">
                <span className="text-gray-400 font-medium text-[11px]">{item.pct}</span>
                <span className="font-extrabold text-gray-900 whitespace-nowrap">{item.val}</span>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
}
