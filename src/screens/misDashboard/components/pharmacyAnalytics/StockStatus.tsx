...
export default function StockStatus() {
  const stockStatuses = [
    { label: "Available", pct: "68%", val: "8,554", color: "bg-emerald-500" },
    { label: "Low Stock", pct: "18%", val: "2,264", color: "bg-amber-400" },
    { label: "Out of Stock", pct: "6%", val: "755", color: "bg-rose-500" },
    { label: "Near Expiry", pct: "5%", val: "628", color: "bg-purple-600" },
    { label: "Expired", pct: "3%", val: "379", color: "bg-red-600" },
  ];
  ...
  let currentDeg = 0;
  const gradientStops = stockStatuses
    .map(item => {
      ...
      <div className="flex flex-col  items-center justify-between my-auto gap-4 sm:gap-0">
        ...
        >
          <div className="w-full h-full bg-white rounded-full flex flex-col items-center justify-center shadow-inner">
            <span className="text-[10px] font-black text-gray-900 leading-tight">12,580</span>
            <span className="text-[8px] text-gray-400 font-medium mt-0.5">Total SKUs</span>
          </div>
        </div>

        {/* Stock Status List with Percentages & Counts */}
          ...
          {stockStatuses.map(item => (
            <div key={item.label} className="flex items-center justify-between text-[11px]">
              ...
              <div className="flex items-center space-x-3 sm:space-x-5">
                ...
                <span className="font-extrabold text-gray-900 w-12 text-right whitespace-nowrap">
                  {item.val}
                </span>
              </div>
            </div>
