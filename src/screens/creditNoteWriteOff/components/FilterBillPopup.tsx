...
import { BillDetailsItem } from "../types";
...
const FilterBillPopup = ({
  ...
  isOpen: boolean;
  onClose: () => void;
  setSelectedBill: Dispatch<SetStateAction<BillDetailsItem | null>>;
}) => {
  const { loading, fetchApi } = useGlobalApi();
  const today = formatToDDMMYYYY(new Date().toISOString().split("T")[0]);
  const [billItemList, setBillItemList] = useState<BillDetailsItem[]>([]);
  ...
  //   bill select handler
  const billSelectHandler = (item: BillDetailsItem) => {
                ...
                <table className="base-table ">
                  ...
                  <tbody>
                    {billItemList.length === 0 ? (
                      ...
                      billItemList.map((item: BillDetailsItem, idx: number) => (
                        <tr
                          key={idx}
